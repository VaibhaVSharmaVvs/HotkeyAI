...
public sealed class WindowsWindows : IWindows
{
    ...
    public ValueTask FocusAsync(WindowRef window, CancellationToken cancellationToken)
    {
        ...
        // SetForegroundWindow is subject to Windows' foreground lock: it can refuse when the
        // calling process did not last receive input. The engine's postcondition
        // (foreground_process_is) is what catches that, which is exactly why it exists.
